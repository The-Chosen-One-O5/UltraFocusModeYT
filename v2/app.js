// ===== BEGIN supabase/client.js (inlined) =====
     ...
     // Get and validate custom duration
           ...
           async function signIn() {
               ...
               // Validation
               if (!email || !password) {
                   showConfirmation("Missing", "Enter Email & Password.", false);
                   return;
               }
               
               // Email validation
               if (!isValidEmail(email)) {
                   showConfirmation("Invalid Email", "Please enter a valid email address.", false);
                   return;
               }
               ...
               try {
                   ...
                   if (error) {
                       showConfirmation("Login Failed", error.message || "Invalid credentials.", false);
                       return;
                   }
                   const user = data?.user;
                   if (!user) {
                       showConfirmation("Login Failed", "No user returned.", false);
                       return;
                   }
             ...
             async function createAccount() {
                 ...
                 if (!username || !email || !password) {
                     showConfirmation("Missing", "Enter Username, Email & Password.", false);
                     return;
                 }
                 ...
                 if (password.length < 6) {
                     showConfirmation("Weak Password", "Password needs at least 6 characters.", false);
                     return;
                 }
                 ...
                 try {
                     ...
                     if (error) {
                         showConfirmation("Sign Up Failed", error.message || "Failed to create account.", false);
                         return;
                     }
                     
                     const user = data?.user;
                     if (!user) {
                         showConfirmation("Sign Up Failed", "No user returned from sign up.", false);
                         return;
                     }
                     ...
                     showConfirmation("Account Created!", `Welcome to the Quest for Focus! Check your email to verify your account.`, false);
                 } catch (err) {
             ...
             function guestSignIn() {
                 ...
                 try {
                     ...
                     console.error("Local storage save error for guest:", err);
                 }
