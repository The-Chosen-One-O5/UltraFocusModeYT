    ...
    <div class="game-container">
        ...
        <div id="signinForm" class="page-view">
            <div class="auth-container">
                ...
                <div class="subtitle">Choose your path to enter the realm.</div>
                
                <!-- Main Authentication Options -->
                <div id="mainAuthView" class="auth-view" style="display: block;">
                    <button id="createAccountBtn" class="auth-btn auth-btn--primary">CREATE ACCOUNT</button>
                    <button id="signInBtn" class="auth-btn auth-btn--primary">SIGN IN</button>
                    <button id="guestSignInBtn" class="auth-btn auth-btn--secondary">SIGN IN AS GUEST</button>
                </div>
                
                <!-- Create Account Form -->
                <div id="createAccountView" class="auth-view" style="display: none;">
                    <div class="subtitle">Create Your Hero Account</div>
                    ...
                    <button id="createAccountSubmit" class="auth-btn auth-btn--primary">CREATE ACCOUNT</button>
                    <button id="backToMain1" class="auth-btn auth-btn--secondary">BACK</button>
                </div>
                
                <!-- Sign In Form -->
                <div id="signInView" class="auth-view" style="display: none;">
                    <div class="subtitle">Sign In to Your Account</div>
                    <input type="email" id="signInEmail" placeholder="Enter Email" required />
                    <input type="password" id="signInPassword" placeholder="Enter Password" required />
                    <button id="signInSubmit" class="auth-btn auth-btn--primary">SIGN IN</button>
                    <button id="backToMain2" class="auth-btn auth-btn--secondary">BACK</button>
                </div>
                
                <!-- Guest Sign In Form -->
                <div id="guestSignInView" class="auth-view" style="display: none;">
                    <div class="subtitle">Sign In as Guest</div>
                    <input type="text" id="guestUsername" placeholder="Enter Username" required />
                    <input type="password" id="guestPassword" placeholder="Enter Password" required />
                    <button id="guestSignInSubmit" class="auth-btn auth-btn--secondary">SIGN IN AS GUEST</button>
                    <button id="backToMain3" class="auth-btn auth-btn--secondary">BACK</button>
                </div>
                
                <button data-action="show-view" data-view="landingPage" class="auth-btn auth-btn--secondary" style="margin-top: 20px;"> <i class="fas fa-arrow-left"></i> Back</button>
            </div>
        </div>
        ...
        <div id="youtubeLecturePage" class="page-view"> <div id="youtubeInputContainer"> <div class="subtitle">Prepare Focus Session</div> <select id="playlistSelect"> <option value="">Load Playlist</option> </select> <div id="urlInputs"> <div class="url-container"> <input type="text" class="youtube-url" placeholder="YouTube URL (Video or Playlist)" required /> </div> </div> <button data-action="add-url"><i class="fas fa-plus"></i> Add URL</button> <div id="playlistManagement"> <input type="text" id="playlistName" placeholder="Save As..." /> <button data-action="save-playlist" class="primary-btn"><i class="fas fa-save"></i> Save</button> <button data-action="remove-playlist" class="accent-btn" style="background: var(--danger); box-shadow: 3px 3px 0 #a71d2a;"><i class="fas fa-trash"></i> Delete</button> </div> <div id="focusButtonsContainer" style="display: flex; justify-content: center; align-items: center; gap: 15px; margin-top: 25px; flex-wrap: wrap;"> <button data-action="start-playback" class="primary-btn"><i class="fas fa-play-circle"></i> BEGIN FOCUS</button> <button id="strictFocusBtn" data-action="start-strict-playback" class="primary-btn" style="background: var(--danger); box-shadow: 3px 3px 0 #a71d2a;"><i class="fas fa-lock"></i> STRICT MODE</button> </div> </div> <div id="playerContainer"> <div id="player"></div> </div> <button id="videoSidebarToggleBtn" title="Toggle List (T)"> <i class="fas fa-list"></i> </button> <div id="youtubeLecturePageControls"> <button id="restartBtn" title="Exit Session"><i class="fas fa-times-circle"></i> Exit</button> </div> <div id="videoSidebar"> <h3><i class="fas fa-video"></i> Playlist</h3> <div id="videoThumbnailList"></div> </div> </div>
        ...
        <div id="pomodoroOverlay" class="dialog-overlay">
            ...
            <div id="pdfView" class="page-view" style="display: none; flex-direction: row; align-items: stretch; justify-content: center; gap: 10px; padding: 10px;">
              <div id="adobe-dc-view" style="height: 100%; flex-grow: 1;"></div>
              <div id="pomodoroTimerSmall" style="width: 200px; display: flex; flex-direction: column; align-items: center; justify-content: center; background: var(--bg-medium); padding: 10px; border-radius: 8px;">
                <h3 style="color: var(--secondary);">Focus Time</h3>
                <div id="pomodoroTimerSmallDisplay" style="font-size: 48px; font-family: 'Press Start 2P', cursive;">60:00</div>
                <button id="exitPdfViewBtn" class="accent-btn" style="background: var(--danger); box-shadow: 3px 3px 0 #a71d2a; margin-top: 20px;"><i class="fas fa-times"></i> EXIT</button>
             </div>
            </div>
                ...
                <div id="pomodoroControls">
                    ...
                    <button id="pomodoroWithPdfBtn"><i class="fas fa-book-open"></i> START WITH PDF</button>
                </div>
            </div>
        </div>
        <!-- Audio Elements -->
